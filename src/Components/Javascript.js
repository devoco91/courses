import React, {useEffect} from 'react'
import "../Styles/Javascript.css";

function Javascript() {

    useEffect(() => {
       
        window.scrollTo(0, 0)
      
    }, [])
  return (
    <div>
         <div className="course">
            <div className="preview">
                <h6>Course</h6>
                <h2>Web Development</h2>
                <a href="/">View previous chapter</a>
            </div>
            <div className="info">
                <div className="progre-wrapper">
                    <div className="progre"></div>
                    <span className="progre-text">
                        2/9 Chapters
                    </span>
                </div>
                <h6>Chapter 2</h6>
                <h2>JAVASCRIPT</h2>
                <p className="p-trunc">Lorem ipsum dolor sit amet consectetur adipisicing elit. Nostrum dicta atque ea earum illum, perferendis suscipit animi assumenda voluptatibus consectetur ratione, neque sapiente voluptatum sit</p>
                {/* <a className="btn" href="/library" >Next Chapter</a> */}
            </div>
        </div>
        <a href="/library" className="float">
{/* <i NameName="fa fa-plus my-float"></i> */}
Next Chapter
</a>
    </div>
  )
}

export default Javascript